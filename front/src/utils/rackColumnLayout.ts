...
export interface RackColumnLayout {
  ...
  layoutType: 'single' | 'double' | 'triple' | 'quadruple'
}
 ...
 * - 161-250cm: 3 columns (equal split)
 */
export function calculateRackColumnLayout(
  totalWidth: number
): RackColumnLayout {
...
export function validateRackLayout(layout: RackColumnLayout): {
  ...
  for (let i = 0; i < layout.columnCount; i++) {
    ...
    if (width < 41 || width > 80) {
      errors.push(
        `Column ${i + 1} width ${width}cm is invalid (must be 41-80cm)`
      )
    }
  }
