...
interface ShoeRackDecorationsProps {
...
/**
 * Shoe decoration types - shoes, sneakers, boots, etc.
 * Each decoration will be used at most once across the entire shoe rack
 */
...
/**
 * Available shoe decoration types
 * Each decoration will be used at most once across the entire shoe rack
 * TODO: Replace with actual shoe 3D models when available
 * For now, using placeholder paths - these should be replaced with:
 * - /assets/3d-models/shoes/sneakers-1.glb
 * - /assets/3d-models/shoes/boots-1.glb
 * - /assets/3d-models/shoes/heels-1.glb
 * etc.
 */
const SHOE_DECORATION_TYPES: Record<string, DecorationType> = {
  //   sneakers1: {
  //     modelUrl: '/assets/3d-models/shoes/sneakers-1.glb', // TODO: Add actual model
  //     scale: [1, 1, 1],
  //     width: 15, // Model width in cm
  //     position: 'left',
  //     offsetX: 3,
  //     userData: { isSneakers: true },
  //   },
  //   boots1: {
  //     modelUrl: '/assets/3d-models/shoes/boots-1.glb', // TODO: Add actual model
  //     scale: [1, 1, 1],
  //     width: 12, // Model width in cm
  //     position: 'center',
  //     userData: { isBoots: true },
  //   },
  //   heels1: {
  //     modelUrl: '/assets/3d-models/shoes/heels-1.glb', // TODO: Add actual model
  //     scale: [1, 1, 1],
  //     width: 10, // Model width in cm
  //     position: 'right',
  //     offsetX: -3,
  //     userData: { isHeels: true },
  //   },
  //   sneakers2: {
  //     modelUrl: '/assets/3d-models/shoes/sneakers-2.glb', // TODO: Add actual model
  //     scale: [1, 1, 1],
  //     width: 16, // Model width in cm
  //     position: 'left',
  //     offsetX: 5,
  //     userData: { isSneakers: true },
  //   },
  //   sandals1: {
  //     modelUrl: '/assets/3d-models/shoes/sandals-1.glb', // TODO: Add actual model
  //     scale: [1, 1, 1],
  //     width: 12, // Model width in cm
  //     position: 'center',
  //     userData: { isSandals: true },
  //   },
  //   boots2: {
  //     modelUrl: '/assets/3d-models/shoes/boots-2.glb', // TODO: Add actual model
  //     scale: [1, 1, 1],
  //     width: 13, // Model width in cm
  //     position: 'right',
  //     userData: { isBoots: true },
  //   },
}
...
/**
 * Determine if a shelf should have shoes (not all shelves should be decorated)
 * Returns true ~70% of the time for visual variety (higher than books since it's a shoe rack)
 */
const shouldDecorateShelf = (seed: number): boolean => {
  return seededRandom(seed + 999) > 0.3 // 70% chance of having shoes
}

/**
 * ShoeRackDecorations - Renders shoe decorations on shoe rack shelves
 *
 * This component:
 * - Calculates the master shelf grid (same logic as RackColumn but with shoe-optimized spacing)
 * - Places shoe decorations at fixed positions on each shelf
 * - Works across all columns, including those with doors
 * - Only decorates SHELVES zones
 */
const ShoeRackDecorationsComponent: React.FC<ShoeRackDecorationsProps> = ({
  ...
  const decorations = useMemo(() => {
    ...
    for (let colIndex = 0; colIndex < columns; colIndex++) {
      ...
      // Calculate MASTER SHELF GRID with shoe-optimized spacing
      const masterGridHeight = desiredHeight
      const MIN_SHELF_SPACING = 18 // Smaller for shoes
      const MAX_SHELF_SPACING = 25 // Smaller for shoes
      const OPTIMAL_SHELF_SPACING = 20 // Optimal for most shoes
      ...
      for (let zoneIndex = 0; zoneIndex < adjustedZones.length; zoneIndex++) {
        ...
        const SHELF_MARGIN = 3 // Smaller margin for shoes
        ...
        shelvesInZone.forEach((shelfPos, shelfIndex) => {
          // Position shoes ON TOP of shelf surface
    ...
    // STEP 2: Filter candidates based on random selection (70% chance for shoes)
    ...
    const decorationKeys = Object.keys(SHOE_DECORATION_TYPES)
    const decorationZ = 5 // Slightly different Z position than books
    ...
    const sortedShelves = [...selectedShelves].sort((a, b) => {
      ...
      if (Math.abs(a.absoluteShelfY - b.absoluteShelfY) < 15) {
    ...
    decorationKeys.forEach((decorKey, decorIndex) => {
      ...
      const decoration = SHOE_DECORATION_TYPES[decorKey]
      ...
      const shoesX = basePositionX + (decoration.offsetX || 0)

      allDecorations.push({
        key: `shoe-decor-col${shelf.colIndex}-${decorKey}`,
  ...
  // Render unique shoe decorations (no repeats) on ~70% of shelves
  return (
    <group>
      <Suspense fallback={<DecorationLoader />}>
        {/* Unique shoe decorations - each type used only once */}
...
export const ShoeRackDecorations = memo(ShoeRackDecorationsComponent)
